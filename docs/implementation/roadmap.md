...
- [~] 阶段 6：JMH harness、隔离 Core 的 ABBA runner、Redis critical 与 Codec 正式 A/B 已落地；
  精确尺寸 RESP 编码优化已通过正式 ABBA，Valkey 全 workload 基线已完成；Redis 全 workload、
  `byte[]` 大 value、系统观测和故障注入待执行
...
后续继续测试 Redis 全 workload，以及 Redis/Valkey 的 `byte[]` 大 value、碎片响应、多 Client 共享 EventLoop、
